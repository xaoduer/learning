...
keytool -export -alias   -file d_client.cer -keystore d_client.jks -storepass 654321
...
openssl rsa -pubin -in test_pub.key -text
...
openssl pkcs8 -topk8 -inform PEM -in client.key -outform PEM -nocrypt > clientpkcs8.key



    openssl genrsa -out ${certDir}${OutPutName}.key 2048
    openssl rsa -in ${certDir}${OutPutName}.key -pubout -out ${certDir}${OutPutName}.pem
    openssl req -new -key ${certDir}${OutPutName}.key -out ${certDir}${OutPutName}.csr  &> /dev/null <<EOF

    openssl x509 -req -CA ${CADir}ca.crt -CAkey ${CADir}ca.key -CAcreateserial -in ${certDir}${OutPutName}.csr -out ${certDir}${OutPutName}.crt -days 3650 -sha256 -extfile openssl.cnf -extensions v3_req
